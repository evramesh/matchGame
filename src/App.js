...
class App extends Component {
  ...
  fruits = () => {
    this.setState({tabId: tabsList[0].tabId})
  }

  animals = () => {
    this.setState({tabId: tabsList[1].tabId})
  }

  places = () => {
    this.setState({tabId: tabsList[2].tabId})
  }

  itemFind = id => {
    const {imgUrl, count} = this.state
    const show = imagesList.filter(eachImg => eachImg.thumbnailUrl === id)
    const {imageUrl} = show[0]
    if (imageUrl === imgUrl) {
      const gettimgurl =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      this.setState({imgUrl: gettimgurl})
  ...
  again = () => {
    ...
    this.setState({active: true, count: 0, time: 60})
    this.timerId = setInterval(this.countdown, 1000)
  }

  render() {
    const {count, time, active, tabId, imgUrl} = this.state
    const result = imagesList.filter(reachItem => reachItem.category === tabId)
    ...
    return (
      <div>
        ...
        {active ? (
          <div className="game">
            <div>
              <img className="back" alt="match" src={imgUrl} />
            </div>
            <ul className="ul">
              <li>
                <button onClick={this.fruits} className="li" type="button">
                  {tabsList[0].displayText}
                </button>
              </li>
              <li>
                <button onClick={this.animals} className="li" type="button">
                  {tabsList[1].displayText}
                </button>
              </li>
              <li>
                {' '}
                <button onClick={this.places} className="li" type="button">
                  {tabsList[2].displayText}
                </button>
              </li>
            </ul>
            <ul className="thumbItem">
              {result.map(each => (
                <li key={each.id} className="li">
                  <button
                    id={each.thumbnailUrl}
                    onClick={() => this.itemFind(each.thumbnailUrl)}
                    type="button"
                  >
                    <img
                      className="thumb"
          ...
          <div className="win">
            ...
            <p> YOUR SCORE</p>
            <p>{count}</p>
            <button className="btn" onClick={this.again} type="button">
              <img
                className="rese"
                alt="reset"
